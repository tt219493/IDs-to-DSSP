...
def ids_to_df(path_to_ids: str, use_lazy : bool = True, ids_only : bool = False, is_parquet : bool = False) -> pl.LazyFrame | pl.DataFrame | list[str]:
    ...
    '''
    if is_parquet:
        df = pl.scan_parquet(path_to_ids)
    else:
        df = pl.scan_csv(path_to_ids, separator='\t')

    df = df.with_columns(
        pl.col('id').str.slice(0, 4).alias('id'),
        pl.col('id').str.slice(5, 3).alias('amino_acid'),
        pl.col('id').str.slice(9).alias('index')
    ).cast({'index' : pl.Int64})
...
=======
                .explode([
                     ...
                     pl.col('index'),
                     pl.col('secondary_structure')]))
>>>>>>> 3ad2846a5420b489a3ff4cbfe317d183bd361fb3
